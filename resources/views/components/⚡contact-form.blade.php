...
<div>
    @if($success)
        <div class="glass-card p-12 text-center border-gold-500/50">
            ...
            <h3 class="text-2xl font-bold mb-4">{{ __('Message Sent') }}</h3>
            <p class="text-slate-400">{{ __('Thank you for reaching out. Our concierge team will contact you shortly.') }}</p>
            <button wire:click="$set('success', false)" class="mt-8 text-xs font-bold uppercase tracking-widest text-gold-500 hover:text-white">{{ __('Send another message') }}</button>
        </div>
    @else
        <form wire:submit.prevent="submit" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    <label class="text-xs uppercase tracking-widest text-gold-500 font-bold">{{ __('Subject') }}</label>
                    <input type="text" wire:model="subject" class="w-full bg-navy-950/50 border border-white/10 rounded-lg px-4 py-3 focus:border-gold-500 focus:ring-0 transition-colors" placeholder="{{ __('How can we help?') }}">
                    @error('subject') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
            </div>

            <div class="space-y-2">
                <label class="text-xs uppercase tracking-widest text-gold-500 font-bold">{{ __('Message') }}</label>
                <textarea wire:model="message" rows="6" class="w-full bg-navy-950/50 border border-white/10 rounded-lg px-4 py-3 focus:border-gold-500 focus:ring-0 transition-colors" placeholder="{{ __('Your message here...') }}"></textarea>
